...
class Command(BaseCommand):
    ...
    def parse_bol_pdf_data(self, pdf_content_bytes, original_pdf_filename="attachment.pdf"):
        ...
        extracted_data = {'actual_compartments': [], 'kpc_loading_order_no': None, 'kpc_shipment_no': None, 'delivery_date': None, 'delivery_time': None, 'vehicle_no': None}
        tmp_pdf_path = None
        try:
            ...
            if header_match:
                ...
                row_pattern = re.compile(r"^\s*(AUTOMOTIVE GASOIL|PREMIUM MOTOR SPIRIT|PMS|AGO)\s+(\d+)\s+([\d,\.]+)\s+([\d,\.]+)\s+([\d,\.]+)\s+([\d\.]+)\s+([\d\.]+)\s+(S\d+)\s*$", re.IGNORECASE | re.MULTILINE)
                current_lon = None
                for match in row_pattern.finditer(text_after_header):
                    try:
                        comp_num = int(match.group(2)); actual_l20_str = match.group(5).replace(',', ''); temp_str = match.group(6).replace(',', '.'); density_str = match.group(7).replace(',', '.'); lon_no_from_row = match.group(8).strip().upper()
                        if not current_lon: current_lon = lon_no_from_row
                        elif current_lon != lon_no_from_row: self.stdout.write(self.style.WARNING(f"    Multiple LONs in table ({current_lon} vs {lon_no_from_row}). Using first: {current_lon}"))
                        extracted_data['actual_compartments'].append({'compartment_number': comp_num, 'quantity_l20': Decimal(actual_l20_str), 'temperature': Decimal(temp_str), 'density': Decimal(density_str)})
                        self.stdout.write(f"    Parsed Comp {comp_num}: L20={actual_l20_str}, Temp={temp_str}, Density={density_str}, LON={lon_no_from_row}")
                    except (ValueError, InvalidOperation) as e_val: self.stdout.write(self.style.WARNING(f"    Could not parse numbers in table row: '{match.group(0)}'. Error: {e_val}"))
                    except Exception as e_row: self.stdout.write(self.style.ERROR(f"    Error parsing table row '{match.group(0)}': {e_row}"))
                if current_lon: extracted_data['kpc_loading_order_no'] = current_lon
        ...
        if not extracted_data.get('kpc_loading_order_no'): self.stdout.write(self.style.ERROR("    Essential: KPC LON is missing after parsing.")); return None
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if status_search != 'OK':
                self.stdout.write(self.style.ERROR(f"Error searching BoL emails: {message_ids_bytes[0].decode()}")); mail.logout(); return

            email_ids_list = message_ids_bytes[0].split()
            if not email_ids_list or email_ids_list == [b'']:
                self.stdout.write(self.style.WARNING(f"No new unread BoL emails matching criteria from sender '{bol_sender_filter}'.")); mail.logout(); return
            ...
            for email_id_bytes in email_ids_list:
                ...
                try:
                    with transaction.atomic(): 
                        ...
                        if not pdf_content: self.stdout.write(self.style.WARNING(f"  No PDF attachment in email {email_id_str}. Skipping.")); continue
                        ...
                        if not parsed_bol_data or not parsed_bol_data.get('kpc_loading_order_no'):
                            self.stdout.write(self.style.ERROR(f"  Failed to parse BoL PDF '{pdf_filename}' or missing LON. Skipping update.")); continue
                        ...
                        if parsed_bol_data.get('actual_compartments'):
                            for bol_comp_data in parsed_bol_data['actual_compartments']:
                                ...
                                try:
                                    ...
                                    lc_to_update.save()
                                    if lc_to_update.quantity_actual_l20 is not None and lc_to_update.quantity_actual_l20 > 0: has_actual_l20_data = True
        ...
        except Exception as e_global: self.stdout.write(self.style.ERROR(f"An unexpected global error in BoL processing: {e_global}"))