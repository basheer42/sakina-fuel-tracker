...
class Command(BaseCommand):
    ...
    def find_trip_by_truck_and_order(self, vehicle_reg_from_bol, kpc_lon_from_bol):
        ...
        try:
            if vehicle_reg_from_bol:
                ...
                logger.info(f"BoL Email Processing: Truck-based matching for vehicle {vehicle_reg_from_bol}")
                ...
                if vehicle_matches.exists():
                    ...
                    logger.info(f"BoL Email Processing: Found vehicle {vehicle.plate_number} for reg {vehicle_reg_from_bol}")
                    ...
                    if active_trips.exists():
                        ...
                        if kpc_lon_from_bol and kpc_lon_from_bol != 'UNKNOWN_LON':
                            if candidate_trip.kpc_order_number == kpc_lon_from_bol:
                                ...
                                logger.info(f"BoL Email Processing: Perfect match Vehicle {vehicle.plate_number} + Order {kpc_lon_from_bol}")
                            else:
                                ...
                                if similarity_ratio > 0.8:  # 80% similarity threshold
                                    ...
                                    logger.warning(f"BoL Email Processing: Truck match with order similarity {similarity_ratio:.2f}")
                                else:
                                    ...
                                    logger.error(f"BoL Email Processing: Truck match but order mismatch - Expected={candidate_trip.kpc_order_number}, BoL={kpc_lon_from_bol}")
                        else:
                            ...
                            logger.info(f"BoL Email Processing: Truck-only match for {vehicle.plate_number}")
                    else:
                        self.stdout.write(f"  ❌ No active trips found for vehicle {vehicle.plate_number}")
                        logger.warning(f"BoL Email Processing: No active trips for vehicle {vehicle.plate_number}")
                else:
                    self.stdout.write(f"  ❌ Vehicle not found: {vehicle_reg_from_bol}")
                    logger.warning(f"BoL Email Processing: Vehicle not found: {vehicle_reg_from_bol}")
            ...
            if not trip_to_update and kpc_lon_from_bol != 'UNKNOWN_LON':
                ...
                logger.info(f"BoL Email Processing: Fallback to order-based matching for {kpc_lon_from_bol}")
                ...
                if smart_result:
                    ...
                    if trip_to_update:
                        ...
                        logger.info(f"BoL Email Processing: Fallback match found Trip {trip_to_update.id}")
            ...
            logger.error(f"BoL Email Processing: Error in truck-based matching: {e}")
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"Searching for BoL emails with criteria: {search_criteria}")
            ...
            for email_id_bytes in email_ids_list:
                ...
                try:
                    with transaction.atomic():
                        ...
                        logger.info(f"BoL Email Processing: Extracted LON={kpc_lon_from_bol}, Vehicle={vehicle_reg_from_bol}")
                        ...
                        if not trip_to_update:
                            ...
                            logger.error(f"BoL Email Processing: Trip not found for LON={kpc_lon_from_bol}, Vehicle={vehicle_reg_from_bol}")
                            continue
                        ...
                        logger.info(f"BoL Email Processing: Trip {trip_to_update.id} matched via {matching_method} with confidence {confidence_score:.2f}")
                        ...
                        if not bol_shipment_no:
                            ...
                            if subject_match:
                                ...
                                logger.info(f"BoL Email Processing: Extracted BoL number from subject: {bol_shipment_no}")
                            else:
                                self.stdout.write(self.style.WARNING("  BoL/Shipment number not found in PDF or email subject"))
                                logger.warning(f"BoL Email Processing: No BoL number found in PDF or subject for Trip {trip_to_update.id}")
                        ...
                        if bol_shipment_no and not trip_to_update.bol_number:
                            ...
                            logger.info(f"BoL Email Processing: Trip {trip_to_update.id} already has BoL number: {trip_to_update.bol_number}")
                        ...
                        if updated_compartments > 0:
                            ...
                            if original_status != 'LOADED':
                                ...
                                self.stdout.write(f"  Trip {trip_to_update.id} already LOADED. Recalculating depletion with L20 data...")
                                logger.info(f"BoL Email Processing: Recalculating depletion for already-LOADED Trip {trip_to_update.id}")
                                ...
                                try:
                                    ...
                                    if reversal_ok:
                                        ...
                                        logger.info(f"BoL Email Processing: Depletion reversal for Trip {trip_to_update.id}: {reversal_msg}")
                                        ...
                                        if depletion_ok:
                                            ...
                                            logger.info(f"BoL Email Processing: New L20 depletion for Trip {trip_to_update.id}: {depletion_msg}")
                                        else:
                                            self.stdout.write(self.style.ERROR(f"    ❌ Failed to create new L20 depletion: {depletion_msg}"))
                                            logger.error(f"BoL Email Processing: L20 depletion failed for Trip {trip_to_update.id}: {depletion_msg}")
                                    else:
                                        self.stdout.write(self.style.ERROR(f"    ❌ Failed to reverse existing depletion: {reversal_msg}"))
                                        logger.error(f"BoL Email Processing: Depletion reversal failed for Trip {trip_to_update.id}: {reversal_msg}")
                                    ...
                                    logger.error(f"BoL Email Processing: Depletion recalculation error for Trip {trip_to_update.id}: {depletion_error}")
                            
                            # Now, save the BoL number after other processing is complete
                            if bol_shipment_no_to_save:
                                Trip.objects.filter(id=trip_to_update.id).update(bol_number=bol_shipment_no_to_save)
                                self.stdout.write(f"  ✅ Saved BoL number after processing: {bol_shipment_no_to_save}")
                                logger.info(f"BoL Email Processing: Saved BoL number after processing: {bol_shipment_no_to_save}")
                        ...
                        processed_successfully_ids.append(email_id_bytes)
                        self.stdout.write(f"  📝 BoL processing completed for Trip {trip_to_update.id} (email will remain unread)")
                        logger.info(f"BoL Email Processing: Completed processing for Trip {trip_to_update.id}, email remains unread")
            ...
            # Emails will remain unread for potential reprocessing.
            self.stdout.write(f"📧 Processed {len(processed_successfully_ids)} BoL email(s) - emails remain unread for potential reprocessing.")
            logger.info(f"BoL Email Processing: Processed {len(processed_successfully_ids)} emails, left unread for reprocessing capability.")
