...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.info(f"Status Email Processing: Connected to mailbox {settings.EMAIL_PROCESSING_MAILBOX}.")
            ...
            if status_search != 'OK':
                ...
                self.stdout.write(self.style.ERROR(f"Error searching status emails: {err_msg}")); mail.logout()
                logger.error(f"Status Email Processing: IMAP search error: {err_msg}")
                return

            email_ids_list = message_ids_bytes[0].split()
            if not email_ids_list or email_ids_list == [b'']:
                self.stdout.write(self.style.WARNING(f"No new unread status emails matching criteria from sender '{status_update_sender}'."))
